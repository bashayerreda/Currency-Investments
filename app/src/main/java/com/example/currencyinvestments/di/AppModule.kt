...
class AppModule {
    ...
    @Provides
    @Singleton
    fun provideBaseErrorHandlingIns() = BaseRetrofitErrorClient()


    @Provides
    @Singleton
    fun provideCoinsRepoImp(coinsApiInterface: CoinsApiInterface,errorClient: BaseRetrofitErrorClient) : CoinRepository{
        return CoinRepositoryImp(coinsApiInterface,errorClient)
